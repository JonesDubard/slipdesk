...
/**
 * Slipdesk — Payroll Page (FIXED)
 * Place at: src/app/(dashboard)/payroll/page.tsx
 *
 * FIXES APPLIED:
 *
 * 1. HOURS NOT CALCULATING — recalcLine() now builds PayrollInput directly
 *    from the line's own fields (regularHours, overtimeHours, holidayHours)
 *    instead of going through employeeToPayrollInput which was ignoring them.
 *
 * 2. DEDUCTIONS COLUMN — PayRunLine now has a `deductions` field.
 *    It is subtracted from net pay AFTER tax and NASSCORP (not from gross).
 *    The column is editable in the grid and shown on the payslip PDF.
 *
 * 3. DASHBOARD GOES WHITE — The AppContext already has the two-flag fix
 *    (booted + dataLoading). This file adds a persistent loading skeleton
 *    guard so navigating away and back never shows a blank page.
 *
 * 4. BULK UPLOAD NOT SAVING — onImport now calls addEmployee() for each
 *    BulkRow so employees are persisted to Supabase, not just held in
 *    local reducer state for the current session.
 *
 * 5. SINGLE EDIT CHANGES OTHER EMPLOYEES — EditableCell now uses the
 *    employee's stable DB id (not array index) as the React key, AND
 *    the local editing state is keyed to `lineId + field` via a useEffect
 *    that resets whenever the value prop changes from outside.
 */
...
// FIX 2: deductions are subtracted from net pay only — they don't affect
// grossPay or the taxable base, so NASSCORP and income tax are unaffected.
// The PayRunLine type in src/lib/mock-data.ts now includes this field.
type GridAction =
...
function bulkRowToPayRunLine(r: BulkRow, exchangeRate: number, realId?: string): PayRunLine {
  ...
  // Use the real Supabase UUID when available so pay_run_lines.employee_id
  // satisfies the FK/RLS constraint. Fall back to a temp id only on error.
  ...
  return {
    ...
    deductions:         r.deductions ?? 0,           // FIX 2
    exchangeRate,
    calc:               null,
...
// ─── FIX 1: recalcLine builds PayrollInput directly from the line's own fields.
//
// The previous version called employeeToPayrollInput() which only read
// `standardHours` and ignored the line's regularHours/overtimeHours/holidayHours.
// Now we build the input object directly — no adapter needed.
// ─────────────────────────────────────────────────────────────────────────────
...
function recalcLine(line: PayRunLine): PayRunLine {
  try {
    const calc = calculatePayroll({
      ...
      rate:               line.rate,
      regularHours:       line.regularHours,       // ← correct field
      overtimeHours:      line.overtimeHours,      // ← correct field
      holidayHours:       line.holidayHours,       // ← correct field
      exchangeRate:       line.exchangeRate,
      additionalEarnings: line.additionalEarnings,
    });

    // FIX 2: Apply deductions to net pay after tax+NASSCORP.
    // We patch calc.netPay here so the rest of the UI (totals, PDF)
    // automatically sees the final take-home amount.
    const deductions = (line.deductions ?? 0);
    if (deductions > 0) {
      const patched = {
        ...calc,
        netPay:          Math.max(0, calc.netPay - deductions),
        totalDeductions: calc.totalDeductions + deductions,
      };
      return { ...line, calc: patched };
    }
...
function gridReducer(state: PayRunLine[], action: GridAction): PayRunLine[] {
  switch (action.type) {
    case "UPDATE_FIELD":
      return state.map((l) =>
        // FIX 5: match by stable id, not index
...
async function generatePayslipBlob({ line, periodLabel, payDate, company }: PdfOptions): Promise<Blob> {
  ...
  const deductionRows = [
    ...
    ...(deductions > 0 ? [{ label:"Other Deductions", note:"Salary advance / loan repayment / etc.", amount:deductions }] : []),
  ];
  ...
  const paymentMethod: string | undefined = line.paymentMethod;
  const methodLabel: Record<string,string> = {
    cash:"Cash", bank_transfer:"Bank Transfer", orange_money:"Orange Money", mtn_momo:"Mobile Money (MTN)",
  };
    ...
    <Document title={`Payslip - ${line.fullName} - ${periodLabel}`} author="Slipdesk">
      <Page size="A4" style={S.page}>
        ...
        <View style={S.erBox}>
          ...
          <View style={S.erRow}>
            <Text style={S.erText}>NASSCORP Employer (6% of {sym}{calc.nasscorp.base.toFixed(2)} regular salary)</Text>
        ...
        {paymentMethod && methodLabel[paymentMethod] && (
          <View style={S.payBox}>
            ...
            <View style={S.payRow}>
              ...
              {paymentMethod === "bank_transfer" && line.bankName && (
                <View style={S.payItem}>
                  <Text style={S.payItemLbl}>Bank</Text>
                  <Text style={S.payItemVal}>{line.bankName}</Text>
                </View>
              )}
              {paymentMethod === "bank_transfer" && line.accountNumber && (
                <View style={S.payItem}>
                  <Text style={S.payItemLbl}>Account Number</Text>
                  <Text style={S.payItemVal}>{line.accountNumber}</Text>
                </View>
              )}
                ...
                <>
                  {line.mobileNumber && (
                    <View style={S.payItem}>
                      ...
                      <Text style={S.payItemVal}>{line.mobileNumber}</Text>
                    </View>
                  )}
                  <View style={S.payItem}>
                    <Text style={S.payItemLbl}>Provider</Text>
                    <Text style={S.payItemVal}>
                      {line.mobileProvider || (paymentMethod === "orange_money" ? "Orange Money" : "Lonestar MTN")}
                    </Text>
                  </View>
                </>
              )}
...
// ─── FIX 5: EditableCell — stable key + reset on external value change ────────
//
// Root cause: `key={i}` on table rows meant React reused cell instances when
// rows reordered or recalculated. Now the cell resets its local draft whenever
// the incoming `value` prop changes (e.g. another cell's edit triggered a
// recalcLine() that updated this row).
// ─────────────────────────────────────────────────────────────────────────────
...
function EditableCell({ value, lineId, field, isLocked, dispatch, prefix="", decimals=2 }: {
  ...
  // FIX 5: sync local draft whenever the external value changes (e.g. another
  // field's edit triggered a full row recalculation that changed this field).
  useEffect(() => {
...
export default function PayrollPage() {
        ...
        if (run && lines.length > 0) {
          const lineRows = lines
            ...
            .map((l) => ({
              ...
              deductions:          l.deductions ?? 0,    // FIX 2
  ...
  // ── FIX 4: onImport persists bulk rows to Supabase via addEmployee() ────────
  //
  // Previously onImport only dispatched IMPORT_ROWS to the local reducer, so
  // the employees vanished on page reload. Now we call addEmployee() for each
  // new row first, then map the saved IDs into PayRunLines.
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  async function handleBulkImport(bulkRows: BulkRow[]) {
    ...
    for (const r of bulkRows) {
      try {
        // Persist to Supabase (upserts on employee_number conflict).
        // addEmployee() now returns the saved Employee with the real Supabase UUID,
        // so we pass that id into the PayRunLine — fixing the FK/RLS violation
        // that occurred when pay_run_lines.employee_id contained a fake BULK-xxx id.
        const saved = await addEmployee({ ...r.employee, isActive: true });
        const line  = bulkRowToPayRunLine(r, exchangeRate, saved?.id);
        payRunLines.push(line);
      } catch (err) {
        console.error("Failed to save bulk employee:", r.employee.employeeNumber, err);
        // Still add to the grid so the run isn't missing people, but without a real id
  ...
  const columns = useMemo(() => [
    ...
    // FIX 2: Deductions column — editable, shown in red, deducted from net pay only
    col.accessor("deductions", { header:"Deductions", size:84,
  ...
  // FIX 5: Use stable employee id as React key (not array index `i`)
  const table = useReactTable({ data:lines, columns, getCoreRowModel:getCoreRowModel(),
    // Stable row IDs prevent React from reusing cell state across rows
    getRowId:(row) => row.id,
  });
                ...
                <div key={run.id} className="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-slate-50">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-400 font-mono">
                      {run.employeeCount} emp · Gross ${run.totalGross.toLocaleString("en-US",{ minimumFractionDigits:2 })}
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-2xl px-5 py-3.5">
        ...
        <p className="text-xs text-blue-600 leading-relaxed">
          ...
          NASSCORP, Income Tax (LRA) and Net Pay recalculate instantly. Add a <strong>Deduction</strong> to subtract from net pay only.
          Use <strong>Import CSV</strong> to add extra rows with hours pre-filled.
        </p>
      </div>
          ...
          <div className="bg-white rounded-t-2xl border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-[#002147] text-white">
              <div className="flex items-center gap-3">
                ...
                {!isLocked && <span className="text-[10px] font-mono text-white/30 hidden sm:inline">Click rate, hours or deductions to edit</span>}
              </div>
              {warningCount > 0 && (
            ...
            {/* Desktop table — FIX 5: key on row.id not index */}
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {table.getRowModel().rows.map((row) => (
                    // FIX 5: key={row.id} which equals line.id (stable DB id)
                    <tr key={row.id}
                      className={`border-b border-slate-100 transition-colors hover:bg-emerald-50/20
                ...
                return (
                  ...
                  <div key={line.id} className={`p-4 ${hasWarn ? "border-l-2 border-l-amber-400" : ""}`}>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-3">
                      ...
                      <div>
                        <p className="text-[10px] font-mono text-slate-400 uppercase mb-0.5">Deductions</p>
                        <EditableCell value={line.deductions ?? 0} lineId={line.id} field="deductions" dispatch={dispatch} isLocked={isLocked} prefix={line.currency==="USD" ? "-$" : "-L$"}/>
                      </div>
                    </div>
                    {line.calc && (
      ...
      {showUpload && (
        <BulkUpload
          ...
          onImport={handleBulkImport}   // FIX 4: calls addEmployee() + saves
        />
      )}
    </div>
  );
