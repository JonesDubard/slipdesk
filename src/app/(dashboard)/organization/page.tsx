    ...
    <Card data-testid={kind === "branches" ? "branches-entities-panel" : undefined}>
      ...
      {kind === "branches" && items.length > 0 && (
        <p style={{ margin: "12px 0 0", fontSize: 11, color: "var(--muted-foreground)", lineHeight: 1.5 }}>
          Next: on <strong style={{ color: "var(--foreground)" }}>Employees</strong>, assign each person to a branch (Unassigned is allowed). CSV import can auto-create unknown branch names.
...
function MultiBranchPanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/org/branch-summary", { credentials: "include" });
        const data = await res.json();
